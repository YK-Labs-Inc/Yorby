...
export default async function SampleAnswersPage({ 
  ...
  async function handleCreateSampleAnswer(formData: FormData) {
    ...
    const result = await createSampleAnswer(params.jobId, params.questionId, formData);
    
    if (result.success) {
      // Redirect to refresh the page with the new sample answer
      redirect(`/dashboard/coach-admin/curriculum/${params.jobId}/questions/${params.questionId}/sample-answers`);
    }
  ...
  async function handleDeleteSampleAnswer(answerId: string) {
    "use server";
    
    const result = await deleteSampleAnswer(params.jobId, params.questionId, answerId);
    ...
    <div className="container mx-auto py-6">
      ...
      <Tabs defaultValue="existing" className="w-full">
        <TabsList className="mb-4">
          <TabsTrigger value="existing">Existing Answers ({sampleAnswers.length})</TabsTrigger>
          ...
          {sampleAnswers.length === 0 ? (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Add sample answers to help students understand what makes a good response to this question.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Button asChild>
                  <Link href={`/dashboard/coach-admin/curriculum/${params.jobId}/questions/${params.questionId}/sample-answers?tab=add`}>
                ...
                <Card key={answer.id} id={`answer-${answer.id}`}>
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-base">Sample Answer {index + 1}</CardTitle>
                      <div className="flex gap-2">
                        <Button asChild size="sm" variant="outline">
                          <Link href={`/dashboard/coach-admin/curriculum/${params.jobId}/questions/${params.questionId}/sample-answers/${answer.id}/edit`}>
                            <Pencil className="h-4 w-4 mr-2" />
                            Edit
                          </Link>
                        </Button>
                        <form action={() => handleDeleteSampleAnswer(answer.id)}>
                    ...
                    <CardDescription>
                      Created on {format(new Date(answer.created_at), "MMMM d, yyyy")}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
        ...
        <TabsContent value="add">
          <SampleAnswerForm 
            ...
            onCancel={() => redirect(`/dashboard/coach-admin/curriculum/${params.jobId}/questions/${params.questionId}/sample-answers`)}
          />
        </TabsContent>
      </Tabs>
    </div>
