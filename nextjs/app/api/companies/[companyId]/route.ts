...
export async function GET(request: NextRequest, context: RouteContext) {
  const logger = new Logger().with({ endpoint: "/api/companies/[companyId]", method: "GET" });
  ...
  try {
    ...
    if (companyError) {
      ...
      logger.error("Database error fetching company", { companyError, companyId });
      return NextResponse.json(
...
export async function PUT(request: NextRequest, context: RouteContext) {
  const logger = new Logger().with({ endpoint: "/api/companies/[companyId]", method: "PUT" });
  ...
  try {
    ...
    if (companyError) {
      ...
      logger.error("Database error updating company", { companyError, companyId });
      return NextResponse.json(
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  const logger = new Logger().with({ endpoint: "/api/companies/[companyId]", method: "DELETE" });
  ...
  try {
    ...
    if (companyError) {
      logger.error("Database error deleting company", { companyError, companyId });
      return NextResponse.json(
