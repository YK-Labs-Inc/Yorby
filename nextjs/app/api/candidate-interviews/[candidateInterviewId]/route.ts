...
export const GET = withAxiom(
  async (
    ...
    context: { params: Promise<{ candidateInterviewId: string }> }
  ) => {
    const { candidateInterviewId } = await context.params;
    ...
    try {
      logger.info("Fetching interview analysis", { candidateInterviewId });
      ...
      if (error?.code === "PGRST116") {
        logger.warn("Analysis not found", { candidateInterviewId });
      ...
      if (!analysis || !analysis.id) {
        logger.error("Analysis not found", { candidateInterviewId });
        return NextResponse.json(
...
export const POST = withAxiom(
  async (
    ...
    const logger = request.log.with({
      method: "POST",
      path: "/api/candidate-interviews/[candidateInterviewId]",
      candidateInterviewId,
    });
