...
export async function fetchStudentEnrolledJobs(
  studentId: string,
  coachId: string
): Promise<EnrolledJob[]> {
...
> {
  ...
  data.forEach((submission) => {
    ...
    const hasCoachFeedback = submission.custom_job_question_submission_feedback?.some(
...
export async function fetchJobMockInterviews(
  jobId: string,
  studentId: string
...
export async function fetchJobCounts(
  jobIds: string[]
): Promise<Record<string, { questionsCount: number; mockInterviewsCount: number }>> {
  ...
  // Count by job ID
  const counts: Record<string, { questionsCount: number; mockInterviewsCount: number }> = {};
  
  jobIds.forEach((jobId) => {
    counts[jobId] = {
      questionsCount: questionCounts?.filter((q) => q.custom_job_id === jobId).length || 0,
      mockInterviewsCount: mockInterviewCounts?.filter((m) => m.custom_job_id === jobId).length || 0,
    };
  });
